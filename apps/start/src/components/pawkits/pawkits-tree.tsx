...
export function PawkitsTree() {
  ...
  const { rootCollections, systemCollections } = useMemo(() => {
    const regular = collections
      .filter((c) => !c.parentId && !c.deleted && !c.isSystem)
      .sort((a, b) => a.position - b.position);

    const system = collections
      .filter((c) => c.isSystem && !c.deleted)
      .sort((a, b) => a.position - b.position);
  ...
  const renderItem = (collection: Collection, level: number = 0) => {
    const children = getChildCollections(collection._id);
    const hasChildren = children.length > 0;
    const isExpanded = expandedIds.includes(collection._id);
