...
export function CreatePawkitForm({ onSuccess, onCancel }: CreatePawkitFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Check for duplicate name (server generates unique slug, but UX is better catching early)
            const expectedSlug = slugify(name);
            const exists = collections.find(c => c.slug === expectedSlug && c.workspaceId === workspace._id && !c.deleted);
            if (exists) {
