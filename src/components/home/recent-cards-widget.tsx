...
export function RecentCardsWidget() {
  ...
  const recentCards = useMemo(() => {
    return cards
      ...
      .slice(0, 20); // Max 20 cards for horizontal scroll
  }, [cards]);
      ...
      <CardContent className="p-3">
        ...
        {recentCards.length > 0 ? (
          <>
            ...
            <div
              className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-thin scrollbar-thumb-bg-surface-3 scrollbar-track-transparent"
              style={{ scrollbarWidth: 'thin' }}
            >
              {recentCards.map((card) => (
                <div key={card.id} className="shrink-0 w-[160px]">
                  <RecentCardItem
                    card={card}
