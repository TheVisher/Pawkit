...
export function WorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  const ensureDefaultRef = useRef(false);
  ...
  useEffect(() => {
    if (ensureDefaultRef.current) return;
    if (!isAuthenticated || isLoading) return;
    if (defaultWorkspace || workspaces.length > 0) return;

    ensureDefaultRef.current = true;
    void ensureDefaultWorkspace()
      ...
      .catch(() => {
        ensureDefaultRef.current = false;
      });
  }, [
    isAuthenticated,
    isLoading,
