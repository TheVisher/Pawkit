...
// Unicode prefixes by field type (user-facing event titles).
...
function generateEventTitle(cardTitle: string, field: string): string {
  ...
  if (normalizedField === 'anniversary') {
    return `${prefix} ${cardTitle}'s Anniversary`;
  }
  if (['renewalday', 'renews'].includes(normalizedField)) {
    return `${prefix} ${cardTitle} Renewal`;
  }
  if (['expirydate', 'expiry', 'warranty'].includes(normalizedField)) {
...
): { freq: string; interval: number } | undefined {
  if (extracted.recurrence === 'yearly') {
    return { freq: 'yearly', interval: 1 };
  }
  if (extracted.recurrence === 'monthly') {
...
export function useSupertagCalendarSync() {
   ...
   */
  const syncCardToCalendar = useCallback(
    async (
      card: Card,
      ...
      const existingByField = new Map(cardEvents.map((e) => [e.source?.field || '', e]));
   ...
   */
  const removeCardEvents = useCallback(
    async (cardId: string, existingEvents: CalendarEvent[]): Promise<number> => {
      const cardEvents = existingEvents.filter(
