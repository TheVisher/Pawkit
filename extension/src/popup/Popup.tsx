...
import { Loader2, Settings, ExternalLink, CheckCircle2, ImageIcon, X, ChevronDown, Library, FolderOpen, LogOut, Key } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Textarea } from '@/components/ui/textarea'
import { Label } from '@/components/ui/label'
import { Card, CardContent, CardHeader } from '@/components/ui/card'
import type { SaveCardMessage, SaveCardResponse, Collection, GetCollectionsResponse, CheckAuthResponse, ValidateTokenResponse } from '@/shared/types'
...
export function Popup() {
  ...
  const handleTokenSubmit = async () => {
    ...
    if (!token) {
      setTokenError('Please enter your extension token')
      return
    }

    if (!token.startsWith('pawkit_ext_')) {
      setTokenError('Invalid token format. Token should start with "pawkit_ext_"')
      return
    }
        ...
        <Card className="border-0 rounded-none shadow-none">
          ...
          <CardContent className="space-y-4 pb-4">
            <div className="text-muted-foreground text-sm text-center">
              Enter your extension token to start saving pages, articles, and links.
            </div>

            <div className="space-y-2">
              <Label htmlFor="token" className="text-xs">Extension Token</Label>
              <Input
                id="token"
                type="password"
                value={tokenInput}
                onChange={(e) => setTokenInput(e.target.value)}
                placeholder="pawkit_ext_..."
