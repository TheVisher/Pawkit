...
// Insert menu groups for the + button
const insertGroups = [
  {
    group: 'Basic blocks',
    items: [
      ...
      { icon: <FileCodeIcon className="size-4" />, label: 'Code', value: KEYS.codeBlock },
  ...
  },
  {
    ...
    items: [
      ...
      { icon: <ChevronRightIcon className="size-4" />, label: 'Toggle list', value: KEYS.toggle },
    ],
  },
  {
    group: 'Media',
    items: [
      { icon: <ImageIcon className="size-4" />, label: 'Image', value: KEYS.img },
    ],
  },
];
...
function Gutter({
  ...
  return (
    <div
      ...
      className={cn(
        'slate-gutterLeft',
        // Base: hidden, show only on hover
        '-translate-x-full absolute top-0 z-50 flex h-full cursor-text',
        // Show on hover (both container and regular blocks)
        getPluginByType(editor, element.type)?.node.isContainer
          ? 'opacity-0 group-hover/container:opacity-100'
          : 'opacity-0 group-hover:opacity-100',
        // Hide completely when selection area is visible
...
const InsertButton = React.memo(function InsertButton() {
  const editor = useEditorRef();
  const element = useElement();
  const [open, setOpen] = React.useState(false);

  const handleInsert = React.useCallback((value: string) => {
    // Focus the block first, then insert
  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen} modal={false}>
      ...
      <DropdownMenuContent
        ...
        side="bottom"
      >
        {insertGroups.map(({ group, items }) => (
          <ToolbarMenuGroup key={group} label={group}>
            {items.map(({ icon, label, value }) => (
              <DropdownMenuItem
                key={value}
                className="min-w-[180px]"
                onSelect={() => handleInsert(value)}
              >
                {icon}
                {label}
              </DropdownMenuItem>
